...
namespace Microsoft.Data.ProviderBase
{
    ...
    sealed internal class DbConnectionPool
    {
        ...
        private void CleanupCallback(Object state)
        {
            ...
            SqlClientEventSource.Log.TryPoolerTraceEvent("<prov.DbConnectionPool.CleanupCallback|RES|INFO|CPOOL> {0}", ObjectID);
            ...
            if (_waitHandles.PoolSemaphore.WaitOne(0, false) /* != WAIT_TIMEOUT */)
            {
                for (; ; )
                {
                    ...
                    Debug.Assert(obj != null, "null connection is not expected");
                    SqlClientEventSource.Log.TryPoolerTraceEvent("<prov.DbConnectionPool.CleanupCallback|RES|INFO|CPOOL> {0}, ChangeStacks={1}", ObjectID, obj.ObjectID);
        ...
        internal void Clear()
        {
            ...
            SqlClientEventSource.Log.TryPoolerTraceEvent("<prov.DbConnectionPool.Clear|RES|CPOOL> {0}, Cleared.", ObjectID);
        }
        ...
        private async ValueTask<DbConnectionInternal> CreateObject(
        ...
        )
        {
            ...
            try
            {
                newObj = await _connectionFactory.CreatePooledConnection(this, owningObject, _connectionPoolGroup.ConnectionOptions, _connectionPoolGroup.PoolKey, userOptions, cancellationToken, async);
                if (null == newObj)
                {
        ...
        internal void DestroyObject(DbConnectionInternal obj)
        {
            ...
            {
                SqlClientEventSource.Log.TryPoolerTraceEvent("<prov.DbConnectionPool.DestroyObject|RES|CPOOL> {0}, Connection {1}, Has Delegated Transaction, waiting to Dispose.", ObjectID, obj.ObjectID);
            }
            else
            {
                SqlClientEventSource.Log.TryPoolerTraceEvent("<prov.DbConnectionPool.DestroyObject|RES|CPOOL> {0}, Connection {1}, Removing from pool.", ObjectID, obj.ObjectID);
                bool removed = false;
                lock (_objectList)
                {
        ...
        async ValueTask WaitForPendingOpen(CancellationToken cancellationToken, bool async)
        {
                    ...
                    while (_pendingOpens.TryDequeue(out next))
                    {
                        ...
                        try
                        {
...
#else
                            {
                                ...
                                (timeout, connection) = await TryGetConnection(next.Owner, delay, allowCreate, onlyOneCheckConnection, next.UserOptions, cancellationToken, async);
                                timeout = !timeout;
                            }
#if DEBUG
                            finally
        ...
        internal async ValueTask<DbConnectionInternal> TryGetConnection(
            DbConnection owningObject,
            DbConnectionOptions userOptions,
            CancellationToken cancellationToken,
            bool async
        )
        {
            ...
            (_, connection) = await TryGetConnection(
                owningObject,
        ...
        [ResourceConsumption(ResourceScope.Machine, ResourceScope.Machine)]
        private async ValueTask<(bool, DbConnectionInternal)> TryGetConnection(
            DbConnection owningObject,
                    ...
                    try
                    {
                        ...
                        switch (waitResult)
                        {
                            ...
                            case CREATION_HANDLE:
                                ...
                                try
                                {
                                    obj = await UserCreateRequest(owningObject, userOptions, cancellationToken, async);
                                }
                                catch
                                {
                                    if (null == obj)
                            ...
                            default:
                                SqlClientEventSource.Log.TryPoolerTraceEvent("<prov.DbConnectionPool.GetConnection|RES|CPOOL> {0}, WaitForMultipleObjects={1}", ObjectID, waitResult);
                                Interlocked.Decrement(ref _waitCount);
        ...
        /// <param name="async">async execution flag</param>
        /// <returns>A new inner connection that is attached to the <paramref name="owningObject"/></returns>
        internal async ValueTask<DbConnectionInternal> ReplaceConnection(
            DbConnection owningObject,
            DbConnectionOptions userOptions,
            DbConnectionInternal oldConnection,
            CancellationToken cancellationToken,
        ...
        )
        {
            ...
            DbConnectionInternal newConnection = await UserCreateRequest(owningObject, userOptions, cancellationToken, async, oldConnection);
        ...
        private void PoolCreateRequest(object state)
        {
            ...
            try
            {
                if (State.Running == _state)
                {
                    ...
                    {
                        Thread waitOpenThread = new Thread(() => _ = WaitForPendingOpen(CancellationToken.None, false))
                        {
                            IsBackground = true
                        };
                                ...
                                if (WAIT_OBJECT_0 == waitResult)
                                {
                                    ...
                                    if (!ErrorOccurred)
                                    {
                                        while (NeedToReplenish)
                                        {
                                            ...
                                            newObj = CreateObject(owningObject: null, userOptions: null, oldConnection: null, CancellationToken.None, false).Result;
        ...
        private bool ReclaimEmancipatedObjects()
        {
            ...
            SqlClientEventSource.Log.TryPoolerTraceEvent("<prov.DbConnectionPool.ReclaimEmancipatedObjects|RES|CPOOL> {0}", ObjectID);
        ...
        private async ValueTask<DbConnectionInternal> UserCreateRequest(
            DbConnection owningObject,
            DbConnectionOptions userOptions,
            CancellationToken cancellationToken,
            bool async,
            ...
            else
            {
                if ((oldConnection != null) || (Count < MaxPoolSize) || (0 == MaxPoolSize))
                {
                    ...
                    if ((oldConnection != null) || (Count & 0x1) == 0x1 || !ReclaimEmancipatedObjects())
                        obj = await CreateObject(owningObject, userOptions, oldConnection, cancellationToken, async);
                }
                return obj;
            }
        }
